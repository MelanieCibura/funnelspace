...
const FS_PREMIUM = 99;
const c3eur = (n) => n.toLocaleString('de-DE') + ' €';

const CalculatorV3 = () => {
  const base = React.useMemo(() => C3_TOOLS.reduce((o, t) => (o[t.key] = t.price, o), {}), []);
  const [prices, setPrices] = React.useState(base);
  const [on, setOn] = React.useState(() => C3_TOOLS.reduce((o, t) => (o[t.key] = true, o), {}));

  const total = C3_TOOLS.reduce((s, t) => s + (on[t.key] ? prices[t.key] : 0), 0);
  const saving = Math.max(0, total - FS_PREMIUM);
  const yearly = saving * 12;
  const count = C3_TOOLS.filter(t => on[t.key]).length;
        ...
        <div className="c2__grid">
          <div className="c2__panel">
            <div className="c2__panel-head">
              ...
              <span>{count} von {C3_TOOLS.length} Tools aktiv</span>
            </div>
                ...
                return (
                  <li className={`c2__row${active ? ' is-on' : ''}`} key={t.key}>
                    <label className="c2__toggle">
                      ...
                      <span className="c2__price">{c3eur(active ? v : 0)}</span>
                    </label>
                    <input
                      className="c2__range"
            ...
            <button type="button" className="c2__reset" onClick={() => { setPrices(base); setOn(C3_TOOLS.reduce((o, t) => (o[t.key] = true, o), {})); }}>
          ...
          <div className="c2__result">
            <div className="c2__compare">
              <div className="c2__compare-item">
                ...
                <div className="c2__compare-price c2__compare-price--old">{c3eur(total)}<small>/mtl.</small></div>
              ...
              <div className="c2__compare-item">
                <span className="c2__compare-label">Funnelspace Komplett</span>
                <div className="c2__compare-price">{c3eur(FS_PREMIUM)}<small>/mtl.</small></div>
            ...
            <div className="c2__save">
              ...
              <div className="c2__save-num">{c3eur(yearly)}</div>
...
window.CalculatorV3 = CalculatorV3;
